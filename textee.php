...
if ($_SESSION['user'] == "admin" || $_SESSION['user'] == "ruc") {
    ...
    if ($anoSelecionado) 
    {
      $stmt = $conexao->prepare("SELECT id, SIGLA, ANO FROM UC WHERE ANO = :ano");
      $stmt->bindValue(':ano', $anoSelecionado);
      $stmt->execute();
      $edicoesUC = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    // Selecionar os dados para a tabela de acordo com o ano letivo e a edição UC selecionados
    if ($_SESSION['user'] == "ruc") 
    {  // "Tabela do RUC"
      $query = "SELECT * FROM INSCREVER_ALUNOS JOIN UC ON INSCREVER_ALUNOS.ED_UC = UC.id WHERE UC.RUC = :ruc AND UC.ANO = :ano";
      if ($edicaoSelecionada) $query .= " AND INSCREVER_ALUNOS.ED_UC = :edicao";
      $resultado = $conexao->prepare($query);
      $resultado->bindValue(':ruc', $_SESSION['user_aka']);
      $resultado->bindValue(':ano', $anoSelecionado);
...
<html lang="pt">
<head>
    <title>Criação de submissões</title>
    <style>
        .container {
            max-width: 1024px;
            margin: 0 auto;
    ...
    <div class="content">
        ...
        <form action="" method="POST">
            ...
            if ($_SESSION['user'] == "ruc") {  // Se for RUC
                ...
                $queryuccreate = $conexao->query("SELECT id, SIGLA, ANO FROM UC");  // Se for ADMIN
            }
            $ucids = array();
            $ucsiglas = array();
            $ucanos = array();
